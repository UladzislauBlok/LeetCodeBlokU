package org.ubdev.task._869_Reordered_Power_of_2;
...
@Topics({HASH_TABLE, MATH, SORTING, COUNTING, ENUMERATION,})
class Solution {
    ...
    private final static Map<Integer, List<int[]>> EXPECTED = preProcess();
    ...
    public boolean reorderedPowerOf2_1(int n) {
        ...
        for (int[] expDigits : EXPECTED.getOrDefault(Arrays.hashCode(digits), List.of())) {
            if (Arrays.equals(digits, expDigits))
                return true;
        }
        return false;
    }
